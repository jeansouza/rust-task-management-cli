...
impl JiraDatabase {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        db.last_item_id = next_id;
        let epic = db.epics.get_mut(&epic_id).ok_or(anyhow!("Error"))?;
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        let mut db = self.database.read_db()?;
        let epic = db.epics.remove(&epic_id).ok_or(anyhow!("Error"))?;
        for story_id in epic.stories {
            db.stories.remove(&story_id).ok_or(anyhow!("Error"))?;
        }
        self.database.write_db(&db)?;
        Ok(())
    }
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        epic.stories.remove(story_index.ok_or(anyhow!("Error"))?);
        self.database.write_db(&db)?;
        Ok(())
    }
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        let mut db = self.database.read_db()?;
        let story = db.stories.get_mut(&story_id).ok_or(anyhow!("Error"))?;
        story.status = status;
        self.database.write_db(&db)?;
        Ok(())
    }
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        ...
        let json: DBState = serde_json::from_str(&file)?;
        Ok(json)
    }
