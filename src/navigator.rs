...
impl Navigator {
    pub fn new(db: Rc<JiraDatabase>) -> Self {
        ...
        navigator.pages.push(Box::new(HomePage {
            db: Rc::new(JiraDatabase::new("data/db.json".to_owned())),
        }));
        navigator
    }
    ...
    pub fn get_current_page(&self) -> Option<&Box<dyn Page>> {
        ...
        Some(&self.pages[self.pages.len() - 1])
    }
        ...
        match action {
            ...
            Action::UpdateEpicStatus { epic_id } => {
                ...
                let status = (self.prompts.update_status)()
                    .with_context(|| format!("Failed to read instrs from {}", "error"))?;
                self.db
