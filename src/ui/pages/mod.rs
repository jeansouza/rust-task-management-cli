    ...
    fn draw_page(&self) -> Result<()> {
        ...
        for id in db.epics.keys().sorted() {
            println!(
                ...
                get_column_string(&db.epics[id].name, 34),
                get_column_string(&db.epics[id].status.to_string(), 17)
            );
        }
    ...
    fn draw_page(&self) -> Result<()> {
        ...
        for id in stories.keys().sorted() {
            println!(
                "{}|{}|{}",
                get_column_string(&id.to_string(), 12),
                get_column_string(&stories[id].name, 34),
                get_column_string(&stories[id].status.to_string(), 17)
            );
        }
